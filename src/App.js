...
class App extends Component {
  ...
  getShuffledImageList = () => {}

  renderImageList = () => {
    const shuffledImageList = this.getShuffledImageList()
  }

  render() {
    const {activeTabId, isGameInProgress, score} = this.state
    console.log(score)
    const filteredProjects = this.getFilteredProjects()
    return (
      <div className="app-container">
        <Header scoreDetails={score} />

        <div>
          {isGameInProgress ? this.renderImageList() : this.renderScoreCard()}
        </div>
        <div>
          ...
          <ul className="images-list-container">
            {filteredProjects.map(imageDetails => (
              <ListItem key={imageDetails.id} imagesList={imageDetails} />
            ))}
          </ul>
        </div>
      </div>
